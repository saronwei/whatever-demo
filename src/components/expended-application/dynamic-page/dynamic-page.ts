...
@Component({
  name: 'demo-dynamic-page',
  template: template,
  style: style,
  components: { DynamicPage, 'demo-previewer': DemoPreviewer, 'demo-list': DemoListComponent, 'demo-deitor': DemoEditor }
})
export class DemoDynamicPageComponent extends Vue {
  /**
   * 允许外部控制动态页面内部的功能交互
   * 现暂时只支持外部呼出元件选择器
   *
   * @type {boolean}
   * @memberof DynamicPageComponent
   */
  @Prop({
    default: false
  })
